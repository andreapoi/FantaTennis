...
def compute_points_with_multipliers(row, bonus_mult_dict, malus_mult_dict):
    ...
    # Bonus/malus da statistiche (se presenti)
    aces = row.get("Aces", 0)
    dfs = row.get("Double Faults", 0)
    ...
    raw = match_pts + round_pts + (aces * ACE_POINT) - (dfs * DF_POINT)
...
# ...
    ...
    if uploaded_results is not None:
      try:
        ...
        # Converte automaticamente CSV "stats" (diretta.it) -> formato classico app
        df_upload = normalize_results_upload(df_upload, upload_season, upload_tournament, upload_type)
    ...
    if df_res.empty:
        ...
        st.markdown("---")
        st.markdown("### 3️⃣ Gestione risultati per singolo torneo")

        if not df_res.empty:
           # scegli torneo
           tournaments = (
           df_res[["Season", "Tournament", "Tournament Type"]]
           .drop_duplicates()
           .sort_values(["Season", "Tournament"])
           )
           if tournaments.empty:
              ...
              if st.button("💾 Applica modifiche di questo torneo al dataset stagione"):
                   ...
                   mask_keep = ~mask_t
                   base = base[mask_keep]
                   base = pd.concat([base, edited_tournament], ignore_index=True)

                   st.session_state.results_df = base
                   st.success(
                              f"Modifiche applicate per {t_name} {sel_season} ({t_type}). "
                              "Ricalcola la classifica stagione per vedere l'effetto."
                              )
